...
def analysis_exercise(title):
    ...
    an_result["如图，在△ABC中，AB=AC，D是BC上一点，E是AB上一点，DB=DE.求证：A、E、D、C四点共圆."] = '''{
        "已知条件": ["triangle(A,B,C)", "AB=AC", "lieson(D,BC)", "lieson(E,AB)", "DB=DE"],
...
def polish_exercise(title):
    ...
    messages.append({"role": "user",
                     "content": "润色以下习题，结果存在JSON数组中，只返回润色后的题干，确保习题的返回顺序与习题的输入顺序一致。\n{}".format(
                         title)})
