        ...
        if max_dis != 0 and min_dis / max_dis >= 1 / 100:
            ...
            for i in range(0, 20):
                ...
                if min_dis <= 1e-10:  # 5.2.1.1  有重合点，先让点不重合
                    key_list = random.sample(list(dis_list.keys()), min(len(dis_list.keys()), 2))  # 不要一次性约束所有的重叠点，容易造成超时和无解
...
def exercise_output(exercise_list: pd):
    ...
    # 3.开始输出习题
    with WolframLanguageSession() as session:
        with open("./new_exercises/new.html", 'w', encoding="UTF-8") as f_thml:
            f_thml.write(r'''<!DOCTYPE html>
                           <head>
                                <meta charset="UTF-8">
                                <title>New Exercises</title>
                ...
                for C in C_list:
                    ...
                    if id == -10:
                        ...
                        Exercise.objects.create(
                            name=exercise_name,
                            content=content,  # 根据您的需求，将习题内容保存到 content 字段中
                            image='/{}.png'.format(exercise_name)  # 图片路径保存到数据库中
                        )
                        break  # 跳出去
